    ...
    return (<>
      <div className={styles.gallery}>
        ...
        <div className={styles.photoGrid}>
          {photos.map((photo) => (<div key={photo.id} className={styles.photoItem} onClick={() => handlePhotoClick(photo)}>
              <img src={getThumbnailUrl(photo)} data-original-path={isStrapiImage(photo) ? photo.url : photo.fullPath} alt={getImageName(photo)} className={styles.photoImage} onError={handleImageError}/>
            </div>))}
        </div>
      </div>

      {selectedPhoto && (<div className={styles.popup} onClick={handleClosePopup}>
          <div className={styles.popupContent} onClick={(e) => e.stopPropagation()}>
            ...
            <img src={getFullImageUrl(selectedPhoto)} alt={getImageName(selectedPhoto)} className={styles.popupImage} tabIndex={0}/>
