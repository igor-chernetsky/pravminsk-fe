...
const ArticlesList = ({ categorySlug, search, date }) => {
    ...
    const loadArticles = async (page, append = false) => {
        try {
            const data = await getArticles({ locale, categorySlug, search, date, page });
            const newArticles = data.data.map((item) => ({
                id: item.id,
    ...
    useEffect(() => {
        ...
        const propsChanged = prevProps.current.categorySlug !== currentProps.categorySlug ||
    ...
    if (loading)
        return <div className={styles.loading}>{t('loading_articles')}</div>;
    if (error)
        return <div className={styles.error}>{t('error') + ': ' + error}</div>;
    if (!articles.length)
        return <div className={styles.empty}>{t('no_articles')}</div>;
    return (<div className={styles.articlesList}>
      {articles.map(article => <ArticleBlock key={article.id} article={article}/>)}
      {hasMore && (<div className={styles.loadMoreContainer}>
