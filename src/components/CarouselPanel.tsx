...
const CarouselPanel: React.FC<CarouselPanelProps> = ({ panels }) => {
    ...
    <div className={styles.carouselContainer}>
      {panels.map((panel, index) => (
        <div
          ...
          onClick={() => panel.link && window.open(panel.link, '_blank')}
          style={{ cursor: panel.link ? 'pointer' : 'default' }}
        >
        ...
        <>
          <div className={styles.carouselControls}>
            {panels.map((_, index) => (
              <button
                key={index}
                className={`${styles.controlButton} ${index === currentSlide ? styles.active : ''}`}
