...
const MainNews = () => {
    ...
    if (loading)
        return <div className={styles.loading}>Загрузка главной новости...</div>;
    if (error)
        return <div className={styles.error}>Ошибка: {error}</div>;
    if (!article)
        return null;
    return (<div className={styles.mainNews}>
      <Link to={`${langPrefix}/article/${article.slug || article.attributes?.slug}`} className={styles.articleLink}>
        <div className={styles.imageContainer}>
          {article.imageUrl ? (<img src={getArticleImageUrl(article.imageUrl)} alt={article.title || article.attributes?.title} className={styles.image}/>) : (<div className={styles.placeholderImage}>
