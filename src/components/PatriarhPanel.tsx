...
interface PatriarhPanelProps {
  panel: Panel;
}

const PatriarhPanel: React.FC<PatriarhPanelProps> = ({ panel }) => {
  ...
  return (
    <div className={styles.patriarhPanel}>
        ...
        <>
          ...
          <nav className={styles.links}>
            {links.map((link) => (
              <Link 
                key={link.name} 
                to={`${langPrefix}${link.url}`} 
                className={styles.link}
              >
                <img 
                  src={`/img/icons/${link.name}.svg`} 
                  alt="" 
