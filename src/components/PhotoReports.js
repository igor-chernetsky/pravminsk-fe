...
const PhotoReports = () => {
    ...
    useRunOnceAsync(async () => {
        ...
        try {
            ...
            const processedArticles = data.data.map((item) => ({
                id: item.id,
                title: item.title || '',
                slug: item.slug || '',
    ...
    if (loading)
        return <div className={styles.loading}>Загрузка фоторепортажей...</div>;
    if (error)
        return <div className={styles.error}>Ошибка: {error}</div>;
    return (<div className={styles.photoReports}>
      ...
      <div className={styles.columnContent}>
        <div className={styles.photoGrid}>
          {articles.map((article) => {
            ...
            return (<Link key={article.id} to={`${langPrefix}/article/${article.slug}`} className={styles.photoItem} onClick={handleArticleClick}>
                {imageUrl ? (<img src={imageUrl} alt={article.title} className={styles.photoImage}/>) : (<div className={styles.photoPlaceholder}>
                    <span>Нет фото</span>
                  </div>)}
              </Link>);
        })}
